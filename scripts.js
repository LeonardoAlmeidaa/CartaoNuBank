...
function cardEffect(event){
    ...
    const postitionX    = event.clientX - centerX;
    const postitionY    = event.clientY - centerY;

    const rotateX       = ((+1)*25*postitionY/(cardHeight/2)).toFixed(2);
    const rotateY       = ((-1)*25*postitionX/(cardWidth/2)).toFixed(2);

    console.log(postitionX,postitionY);
